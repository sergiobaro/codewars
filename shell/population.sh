...
# the function nb_year should return n number of entire years needed to get a population greater or equal 
# to p.
...
nbYear() {
    local initial_population=$1
    local growth_percentage=$( echo "$2/100" | bc -l )
    local variation=$3
    local target=$4

    local n=1
    local population=$( echo "$initial_population + $variation + $initial_population * $growth_percentage" | bc -l )
    population=${population%.*}

    while (( $( echo "$population < $target" | bc -l ) )); do
        n=$(( $n + 1 ))
        population=$( echo "$population + $variation + $population * $growth_percentage" | bc -l )
        population=${population%.*}
    done
...
test() {
    echo "p0: $1, perc: $2, aug: $3, p: $4"
    local result=$( nbYear $1 $2 $3 $4 )
    local expected=$5
...
test "1000" "2.0" "50" "1214" "4"
test "1500000" "2.5" "10000" "2000000" "10"
test "1500000" "0.0" "10000" "2000000" "50"